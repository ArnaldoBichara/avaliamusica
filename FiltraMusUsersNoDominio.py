...
#           nova_row = row[['userid','interpretacao']].copy()
#           nova_row['id_musica']=id_musica
#           newmususers = newmususers.append(nova_row, ignore_index=True)     
            dfMusUsers.at[index,'id_musica']=id_musica
#
#logging.info('MusUsers, depois do filtro %s', newmususers.shape)
logging.info('MusUsers, depois do filtro %s', dfMusUsers.shape)

# salvando dataset
#newmususers.to_pickle ("./FeatureStore/MusUsers.pickle")
dfMusUsers.to_pickle ("./FeatureStore/MusUsers.pickle")
logging.info('<< FiltraMusUsersNoDominio')
