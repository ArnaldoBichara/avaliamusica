...
dfUserA_MusCurte = dfUserA_MusCurte.drop(columns=['id_musica'], inplace=True, ignore_index=True)
dfUserA_MusNaoCurte = dfUserA_MusNaoCurte.drop(columns='id_musica', inplace=True, ignore_index=True)
#%%
logging.info ('MusUsers antes = %s', df600kMusCurtem.shape)
...
df600kMusCurtem.drop_duplicates(inplace=True, ignore_index=True)

# %%
df600kMusCurtem.to_pickle('./FeatureStore/MusUsers.pickle')

logging.info ('MusUser após concatenar com UserA = %s', df600kMusCurtem.shape)
